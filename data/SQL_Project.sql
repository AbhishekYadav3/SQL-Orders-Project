...
CASE WHEN p.PRODUCT_PRICE > 20000 THEN p.PRODUCT_PRICE * 0.8
...
CONCAT(oc.customer_fname, ' ', oc.customer_lname) AS full_name, oc.customer_email,
oc.customer_phone, addr.country
FROM ONLINE_CUSTOMER oc
JOIN ADDRESS addr ON oc.address_id = addr.address_id
WHERE oc.customer_id IN (SELECT oh.customer_id FROM ORDER_HEADER oh WHERE oh.order_status = 'Cancelled'
...
## Answer 6
SELECT p.PRODUCT_ID, p.PRODUCT_DESC, p.PRODUCT_QUANTITY_AVAIL, COALESCE(oi.quantity_sold, 0) AS quantity_sold,
CASE WHEN pc.PRODUCT_CLASS_DESC IN ('Electronics', 'Computer') THEN
CASE WHEN COALESCE(oi.quantity_sold, 0) = 0 THEN 'No Sales in past, give discount to reduce inventory'
WHEN p.PRODUCT_QUANTITY_AVAIL < 0.1 * COALESCE(oi.quantity_sold, 0) THEN 'Low inventory, need to add inventory'
WHEN p.PRODUCT_QUANTITY_AVAIL < 0.5 * COALESCE(oi.quantity_sold, 0) THEN 'Medium inventory, need to add some inventory' ELSE 'Sufficient inventory'
END
...
END
ELSE CASE WHEN COALESCE(oi.quantity_sold, 0) = 0 THEN 'No Sales in past, give discount to reduce inventory'
...
LEFT JOIN (SELECT PRODUCT_ID, SUM(PRODUCT_QUANTITY) AS quantity_sold FROM order_items
...
## Answer 7.
SELECT oi.ORDER_ID, MAX((p.LEN * p.WIDTH * p.HEIGHT) * oi.PRODUCT_QUANTITY) AS Max_Order_Volume
FROM order_items oi
...
## Answer 9.
Select p.product_id, p.product_desc,sum(p.product_quantity_avail) as total_quantity
from product as p 
inner join order_items as o on o.product_id = p.product_id
inner join order_header as oh on o.order_id = oh.order_id
inner join online_customer as oc on oc.customer_id = oh.customer_id
inner join address as a on a.address_id = oc.address_id
where o.order_id in (select o.order_id
from product as p
inner join
order_items as o on o.product_id = p.product_id
where p.product_id = 201) and a.city not in ('Bangalore','New Delhi') and p.product_id != 201
...
oi.order_id, sum(oi.product_quantity) total_quantity from
address a join online_customer c using(address_id)
join order_header oh using(customer_id)
join order_items oi using(order_id)
where oh.order_status="Shipped" and
mod(oi.order_id,2)=0 and left(a.pincode,1)!= "5"
group by c.customer_id,customer_fullname,oi.order_id;    
